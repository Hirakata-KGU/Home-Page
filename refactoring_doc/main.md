...
### 2.3 情報系ページ（`/info/`）と画像配信設計 (`@nuxt/image`)
- **ルーティング構造:** `about.vue`, `contact.vue`, `faq.vue`, `pamphlet.vue` を `app/pages/info/` 配下に集約。
- **「平潟祭について」([`info/about.md`](./info/about.md)):**
  - 委員長挨拶は余計な枠線や写真スペースを排し、公式挨拶文をストレートに読みやすく展開。
  - 2025年度の学園祭写真は大判クロスフェードスライドショーとして配置。
- **画像最適化方針:** 原則として `@nuxt/image`（`<NuxtImg>`）を採用し、`format="webp"` とレスポンシブ `sizes` 指定による高速配信を実現。芸能写真等で非圧縮が求められる場合のみ素の `<img>` を使用。
